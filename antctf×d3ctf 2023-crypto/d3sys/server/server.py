...
class D3_SYS(socketserver.BaseRequestHandler):
    ...
    def register(self):
        self.send(b'Welcome to D^3 CTF 2023!')

        username = self.recv(b"[D^3] USERNAME:").decode()
        if username in self.UsernameDict:
            self.send(f'[D^3] Sorry,the USERNAME {username} has been registered.'.encode())
            return 

        elif len(username) >= 20:
            self.send(b'[D^3] Sorry,the USERNAME can\'t be longer than 20.')
            return
        ...
        self.send(('[D^3] ' + username + ', token is ' + cipher_token.hex() + '& nonce is ' + nonce).encode())
    ...
    def login(self):
        ...
        if username not in self.UsernameDict:
            self.send(b'[D^3] Sorry,the username has never been registered.')
            return False

        veritag = self.UsernameDict[username]
        cipher_token = bytes.fromhex(self.recv(b"[D^3] Token:").decode())
        try:
            msg,tag = self.d3block.decrypt(cipher_token)
        except:
            self.send(b"[D^3] Something Wrong...quiting....")
            return False

        if tag != veritag :
            self.send(b'[D^3] Ouch! HACKER? Get Out!')
            return False

        try:
            token_dict = json.loads(msg.decode('latin-1').strip().encode('utf-8'))
        except:
            self.send(b'[D^3] Sorry,try again plz..')
            return False

        ID = token_dict["id"]
        if ID != username:
            self.send(b'[D^3] Change name?')
            return False

        elif abs(int(time.time()) - token_dict['time']) >= 1:
            self.send(b'[D^3] oh...no...out....')
            return False

        elif token_dict['admin'] != True:
            self.send(b"[D^3] Logining.")
            return False
    ...
    def time(self):
        self.send(b'[D^3] D^3\' clock shows '+str(int(time.time())).encode())
        return
    ...
    def handle(self):
        ...
        self.send(b'[D^3] My initial Authdate is ' + authdate.hex().encode())
        self.send(b'[D^3] My Auth pubkey is ' + str(self.d3block.crt_rsa.pub).encode())
        self.UsernameDict = {}
        admin = None
        signal.alarm(60)
        while 1:
        ...
        if admin != True:
            self.send(b"ByeBye! You are not admin.......")
            self.request.close()
            return

        self.send(b"Hello,Admin.Now, you have 2 chances to operate.")
        for __ in 'D^3 CTF'[:2]:
            self.send(MENU2)
            option = self.recv(b'option >')
            if option == b'F':
                cip = self.enc_flag()
                self.send(b'Encrypted Flag: ' + hex(cip).encode())
            elif option == b'G':
