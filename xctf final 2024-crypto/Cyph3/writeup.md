...
”G“是给出0x139个python random中的不连续的随机数产生的明文通过ToyCipher对应的密文；

”E“是给0x32次选择明文加密ToyCipher；

”F“是需要对随机数生成的下一组明文加密后进行预测

其实攻击思路明显可以得知：需要对ToyCipher进行分析后通过选择明文攻击恢复密钥，通过密钥恢复”G“中对应的明文后，对MT19937进行非连续数的恢复状态后预测下一组明文并对其加密即可。

首先是需要对ToyCipher观察，发现它的sbox有一条相对而言比较好看的高概率差分对（00000006 => 00000001 : 1/4）
...
class ToyCypher:
    ...
    def s(self, x ,index):
        return s_box[index][(((x >> 4) & 3) << 4) + (x & 15)]

    def p(self,x):
        bx = [int(_) for _ in bin(x)[2:].rjust(32, '0')]
        by = [bx[p_box[i]] for i in range(32)]
    ...
    def new_F(self,x, keyi):
        x_in = bin(self.E(x) ^ keyi)[2:].rjust(48, '0')
        y_out = ''
        for i in range(0, 48, 6):
            y_out += bin(self.s(int(x_in[i:i+6], 2), i // 6))[2:].rjust(4, '0')
        y_out = int(y_out, 2)
    ...
    def search_diff(self):
        ...
        P_list = [0]+[1<<i for i in range(32)]+[3<<i for i in range(31)]
        ...
        for P_ in P_list:
            ...
            for i in range(5000):
                ...
                p2 = p1 ^ P_
                k1 = getRandomNBitInteger(48)
                X_ = self.new_F(p1,k1)^self.new_F(p2,k1)
                counter[X_] += 1

            X_, freq = counter.most_common(1)[0]
            # print(counter.most_common(3))
            if freq > 1000:
                print(hex(P_)[2:].rjust(8,'0'),'=>',hex(X_)[2:].rjust(8,'0'),':', freq / 5000) 

import os
toycip = ToyCypher(b2i(os.urandom(6)))
toycip.search_diff()
```

ToyCypher是5round的，因此可以使用上述差分对去构造一组差分路径进行差分攻击恢复出来密钥
...
成功恢复出密钥之后就是对G得到的密文解密后，使用MT19937不连续的段进行恢复state，后预测加密即可，整体code如下
