...
def sha3_multirate_padding(used_bytes, align_bytes):
    """
    The SHA-3 padding function.
    SHA-3 uses domain separation suffix 0x06 instead of 0x01.
    """
    padlen = align_bytes - used_bytes
    if padlen == 0:
        padlen = align_bytes
    # SHA-3 uses 0x06 as domain separation
    if padlen == 1:
        return [0x86]  # 0x06 | 0x80
    else:
        return [0x06] + ([0x00] * (padlen - 2)) + [0x80]
 
def keccak_f(state, rounds = 24):
    """
    ...
    nr = rounds# 3# 12 + 2 * int(log(state.lanew, 2))
...
class KeccakSponge:
    def __init__(self, bitrate, width, padfn, permfn, rounds=24):
    ...
    def absorb_block(self, bb):
        ...
        self.permfn(self.state,self.rounds)
...
class KeccakHash:
    ...
    def __init__(self, bitrate_bits, capacity_bits, output_bits,rounds=24, module_name="Keccak"):
        # our in-absorption sponge. this is never given padding
        assert bitrate_bits + capacity_bits in (25, 50, 100, 200, 400, 800, 1600)
        padding = sha3_multirate_padding if module_name == "SHA3" else multirate_padding
        self.sponge = KeccakSponge(
            bitrate_bits, bitrate_bits + capacity_bits, padding, keccak_f, rounds
        )
    ...
    @staticmethod
    def preset(bitrate_bits, capacity_bits, output_bits, rounds=24, module_name="Keccak"):
        """
        ...
        def create(initial_input=None):
            h = KeccakHash(bitrate_bits, capacity_bits, output_bits, rounds, module_name)
...
Keccak224 = KeccakHash.preset(1152, 448, 224, 4) 
